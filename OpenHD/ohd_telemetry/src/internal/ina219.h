...
class INA219 {
 public:
   ...
   */
  INA219(float shunt_resistance, float max_expected_amps,
         uint8_t address);  // Custom device address and amps
 ...
 private:
  ...
  void calibrate(int bus_volts_max, float shunt_volts_max,
  ...
  */
  ...
  /**
   * @brief Reads the bus voltage register from the sensor and converts to
   * Volts.
   *
  ...
  /**
   * @brief Reads the shunt voltage register from the sensor and converts to
   * millivolts.
   *
   * @return float Shunt voltage in millivolts (mV). "Inf" or "NaN" when
   ...
   *
   * @return float Supply voltage in volts (V). "Inf" or "NaN" when overflow is
   * detected.
   */
  float supply_voltage();

  /**
   * @brief Reads the current register from the sensor and converts to
   * milliamps.
   *
   * @return float Current in milliamps (mA) going through the sensor. "Inf" or
   * "NaN" when overflow is detected.
   */
  float current();

  /**
   * @brief Reads the bus power register from the sensor and converts to
   * milliwatts.
   *
   * @return float Bus power consumption in milliwatts (mW).  "Inf" or "NaN"
 ...
 public:
  // Set this to true if any error occurs in the lib calls (it is not required
  // to have an ina219 connected)
